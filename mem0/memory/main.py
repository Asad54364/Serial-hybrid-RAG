...
class Memory(MemoryBase):
    ...
    def entity_store(self):
        ...
        if self._entity_store is None:
            ...
            # *_entities uses the same Qdrant client; mirror dense/sparse (BM25) capability from the
            # primary store so toggles like ``_has_bm25_slot = False`` (CLI / crash workarounds) apply here too.
            if hasattr(self.vector_store, "_has_bm25_slot") and hasattr(self._entity_store, "_has_bm25_slot"):
                self._entity_store._has_bm25_slot = self.vector_store._has_bm25_slot
    ...
    def get_recent_messages(
        ...
        *,
        ...
        """
        Return the last ``limit`` SQLite-backed conversation turns for this session scope.

        Short-term/working memory is keyed the same way as ``add()`` (``user_id`` / ``agent_id`` / ``run_id``).
        """
        _validate_search_params(top_k=limit)
        ...
        return self.db.get_last_messages(session_scope, limit)
    ...
    def delete_recent_messages(
        ...
        *,
        ...
        """
        Delete all SQLite-backed conversation turns for this session scope.
        """
        _, effective_filters = _build_filters_and_metadata(
            user_id=user_id,
            agent_id=agent_id,
...
class AsyncMemory(MemoryBase):
    ...
    async def get_recent_messages(
        ...
        *,
        ...
        def _sync_get_recent() -> List[Dict[str, Any]]:
            _validate_search_params(top_k=limit)
            _, effective_filters = _build_filters_and_metadata(
                user_id=user_id,
                agent_id=agent_id,
        ...
        return await asyncio.to_thread(_sync_get_recent)

...
class SQLiteManager:
    ...
    def delete_messages(self, session_scope: str) -> None:
        """Delete all messages for a specific session scope."""
        with self._lock:
            try:
                self.connection.execute("BEGIN")
                self.connection.execute("DELETE FROM messages WHERE session_scope = ?", (session_scope,))
                self.connection.execute("COMMIT")
                logger.info(f"Deleted messages for session scope: {session_scope}")
            except Exception as e:
                self.connection.execute("ROLLBACK")
                logger.error(f"Failed to delete messages for session scope {session_scope}: {e}")
                raise

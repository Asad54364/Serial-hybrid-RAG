...
class Qdrant(VectorStoreBase):
    def __init__(
        ...
        if client:
            self.client = client
            # A ready-made client is used for embedded (path) deployments when Memory shares one
            # client across the main vector store and the *_entities collection. In that case
            # host/url/api_key are unset and treating is_local=False incorrectly runs
            # create_payload_index on embedded Qdrant (no-op + UserWarning each time ``create_col`` runs).
            self.is_local = not bool(url or api_key or (host and port))
        else:
            params = {}
            if api_key:
